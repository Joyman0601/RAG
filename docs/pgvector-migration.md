# 把内存向量库换成 pgvector：一次「守住简历声称」的工程改造
...
面试官只要问一句「你这向量库怎么选型的？百万级文档检索多久？索引怎么建的？」——线性扫描答不上来。
这是简历最大的可信度缺口，所以优先级最高（我们内部叫它 P0）。
...
面试时这一段可以讲：「我把 visibility 的五种可见性翻译成 SQL 的 OR 分支，租户隔离是 WHERE 第一条，
CUSTOM 白名单用 PG 数组的包含/交集运算符配 GIN 索引，全程参数化防注入。」——这比「我做了权限隔离」具体得多。
...
这就是「**召回率 vs 延迟**」的权衡——HNSW 的所有参数本质都在调这个跷跷板。面试问「百万级怎么办」，
